...
namespace
{
    ...
    public:
        CartoonRenderer(unsigned horizontal_size, unsigned vertical_size, unsigned shade_count, raytracer::Sampler sampler, RayTracer ray_tracer, tasks::TaskScheduler scheduler)
            : RendererBaseImplementation(horizontal_size, vertical_size, sampler, ray_tracer, scheduler), shade_count(shade_count)
        {
            // NOP
        }
        ...
        Color render_pixel(const math::Rasterizer& window_rasterizer, const Position2D& position, const Scene& scene) const
        {
            ...
            m_sampler->sample(pixel_rectangle, [this, &c, &sample_count, &scene](const Point2D& sample_position) {
                scene.camera->enumerate_rays(sample_position, [this, &c, &sample_count, &scene](const Ray& ray) {
                    c += m_ray_tracer->trace(scene, ray).color.quantized(shade_count);
                    ++sample_count;
                    });
                });
