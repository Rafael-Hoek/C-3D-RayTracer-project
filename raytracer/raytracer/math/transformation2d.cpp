...
Transformation2D math::transformations::rotate(Angle a)
{
	Matrix3x3 tm = transformation_matrices::rotation(a);
    Matrix3x3 itm = transformation_matrices::rotation(-a);
	return Transformation2D(tm, itm);
}

Transformation2D math::transformations::scale(const double sx, const double sy)
{
	const auto tm = transformation_matrices::scaling(sx, sy);
    const auto itm = transformation_matrices::scaling(1 / sx, 1 / sy);
